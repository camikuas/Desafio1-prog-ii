...
public class Puzzle : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.F1) && inventario.ContainsKey("Diamante"))
        {
            UsarInventario(inventario["Diamante"]);
        }

        if (Input.GetKeyDown(KeyCode.F2) && inventario.ContainsKey("Oro"))
        {
            UsarInventario(inventario["Oro"]);
        }
        ;
        if (Input.GetKeyDown(KeyCode.F3) && inventario.ContainsKey("Calabaza"))
        {
            UsarInventario(inventario["Llave"]);
        }
    }
